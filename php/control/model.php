...
class Model
{
	...
	protected $this->__include_this_collections_in_JSON = array();
	...
	public function load_from_db( $ID = null, $store = null )
	{
		$row = $this->__db->get_row( "SELECT * FROM " . $this->get_table() . " WHERE `ID`='{$ID}'" );
		$this->load_from_object( $row, false );
// 		$this->fetch_collections( $store );
	}
	public function fetch_collections( $store = null )
	{
		$ID = $this->ID;
		foreach( (array)$this->__collections as $c )
		{
			...
			{
				if( is_object( $store ) and $store->has_item( $c, $c_ID ) )
				{
					$e = $store->get_item( $c, $c_ID );
				}
				else
				{
					$e = new $c;
	...
	public function toJSON()
	{
			...
			{
				if( !in_array( $collection, $this->__include_this_collections_in_JSON )
					continue;
				...
				$output .= "\"{$collection}\":" . $this->get_collection( $collection, true );
			}
			$output .= "}";
			return $output;
	}
...
// 		echo( $collection );
	...
	public function register_to_parents( $store )
	{
		foreach( $this->__parent_ids as $p=>$v )
		{
			$parent = $store->get_item( $p, $this->get($v) );
